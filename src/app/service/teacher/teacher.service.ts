...
export class TeacherService {
  ...
  getTeacherInformation(userInfo_id: string): Observable<{ userInfo: Teacher }> {
    return this.http.get<{ userInfo: Teacher }>(environment.apiUrl + 'teacher/get-teacher-info', {
  ...
  getReminderList(userInfo_id: string, std_id : string, exam_id : string)
    : Observable<{ reminder: number, next: {std_id : string, exam_id : string} }> {
      return this.http.get<{ reminder: number, next: {std_id : string, exam_id : string} }>(environment.apiUrl + 'teacher/get-reminder-list', {
  ...
  getStudentInfo(userInfo_id: string, page: number, perPage: number, type: number, condition: string)
    ...
    return this.http.get<{ userInfo: TeacherStudentInfo[], total: number }>(environment.apiUrl + 'teacher/get-student-list', {
  ...
  getUserAnswer(std_id: string, paper_id: string, exam_id: string): Observable<{ answer: result, std_name: string, exam_status: number, college: string }> {
    return this.http.get<{ answer: result, std_name: string, exam_status: number, college: string }>(environment.apiUrl + 'teacher/get-user-answer', {
  ...
  getPaperEntry(exam_id: string): Observable<paperModel> {
    return this.http.get<paperModel>(environment.apiUrl +'teacher/get-paper', {
  ...
  postChangeMarkForStudent(std_id: string, exam_id: string, index: number, mark: number, type: number): Observable<{ status: boolean }> {
    ...
    return this.http.post<{ status: boolean }>(environment.apiUrl + 'teacher/change-mark', submitData)
      .pipe(
        tap(() => {
  ...
  submitStudentMark(std_id: string, exam_id: string, name: string) {
    ...
    return this.http.post<{ status: boolean }>(environment.apiUrl + 'teacher/submit-mark', submitData)
      .pipe(
        tap(() => {
          this.teacherSubject.next()
        })
